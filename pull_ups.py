...
frame_threshold = 2  # slightly more sensitive so reps register
up_frames = 0
down_frames = 0
partial_frames = 0
last_rep_time = 0.0
min_time_between_reps = 0.6
wrong_alert_start = -1.0
partial_rep_detected = False
movement_started = False
movement_idle_frames = 0
movement_threshold = 0.025  # minimum vertical drop to consider a rep started
workout_start_time = time.time()
is_paused = False
paused_time = 0.0
pause_start_time = 0.0
baseline_shoulder_y = None
bar_y = None  # estimated bar height (auto-updated from wrist when hanging)
bar_smooth_factor = 0.2
bar_candidates = deque(maxlen=30)
bar_lock_frames = 0
bar_locked = False
bar_lock_required = 12  # frames of stable reading to lock
bar_max_variance = 0.0003  # variance threshold to accept lock
...
def register_incorrect_rep(n: int = 1):
    ...
    if consecutive_incorrect >= 3:
        ...
        instruction_text = (
            "Proper Pull-Up Form:\n"
            "- Pull until your chin clears the bar; avoid kicking or swinging.\n"
            "- Elbow angle should come below ~90 degrees at the top of the pull.\n"
            "- Control the descent; do not drop quickly.\n"
            "- Keep shoulders engaged; avoid shrugging.\n\n"
            "Learn more:\n"
            "https://www.exrx.net/WeightExercises/Lats/BWPullup"
        )
...
recording_name = f"pullup_session_{datetime.now().strftime('%Y%m%d_%H%M%S')}.mp4"
out = cv2.VideoWriter(recording_name, fourcc, 20.0, (WINDOW_WIDTH, WINDOW_HEIGHT))

cv2.namedWindow('FitMaster AI - Pull-Up Counter')
cv2.setMouseCallback('FitMaster AI - Pull-Up Counter', mouse_callback)
    ...
    while cap.isOpened():
        ...
        try:
            ...
            if landmarks:
                ...
                if baseline_shoulder_y is None and stage == "down":
                    baseline_shoulder_y = avg_shoulder_y
                elif baseline_shoulder_y is not None:
                    baseline_shoulder_y = max(baseline_shoulder_y, avg_shoulder_y)

                elbow_x = int(((l_elbow[0] + r_elbow[0]) / 2) * VIDEO_WIDTH)
                elbow_y = int(((l_elbow[1] + r_elbow[1]) / 2) * VIDEO_HEIGHT)
                cv2.putText(video_image, f"{int(elbow_angle)}°",
                            (elbow_x + 20, elbow_y),
                            cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0, 255, 255), 2)

                # Auto-estimate bar height from wrists when arms are extended (hanging and stable)
                hanging = elbow_angle > 150
                if hanging:
                    wrist_bar_candidate = min(l_wrist[1], r_wrist[1])
                    bar_candidates.append(wrist_bar_candidate)
                    if len(bar_candidates) >= 8:
                        avg_candidate = sum(bar_candidates) / len(bar_candidates)
                        variance = sum((c - avg_candidate) ** 2 for c in bar_candidates) / len(bar_candidates)
                        if variance < bar_max_variance:
                            bar_lock_frames += 1
                        else:
                            bar_lock_frames = 0
                        if bar_lock_frames >= bar_lock_required:
                            bar_locked = True
                        # Smooth even before lock, but trust after lock
                        if bar_y is None:
                            bar_y = avg_candidate
                        else:
                            bar_y = (1 - bar_smooth_factor) * bar_y + bar_smooth_factor * avg_candidate
                else:
                    bar_lock_frames = 0

                if bar_y is not None:
                    bar_line = int(bar_y * VIDEO_HEIGHT)
                    cv2.line(video_image, (0, bar_line), (VIDEO_WIDTH, bar_line), (0, 0, 255), 2)
                    label = "Bar (locked)" if bar_locked else "Bar (estimating...)"
                    cv2.putText(video_image, label, (10, bar_line - 8),
                                cv2.FONT_HERSHEY_SIMPLEX, 0.6, (0, 0, 255), 2)

                if not is_paused and baseline_shoulder_y is not None:
                    vertical_drop = baseline_shoulder_y - avg_shoulder_y  # positive when pulling up
                    nose_y = landmarks[mp_pose.PoseLandmark.NOSE.value].y if landmarks else avg_shoulder_y

                    # Detect start of meaningful movement
                    if vertical_drop > movement_threshold or elbow_angle < 135:
                        movement_started = True
                        movement_idle_frames = 0
                    else:
                        movement_idle_frames += 1
                        if movement_idle_frames > frame_threshold * 3:
                            movement_started = False
                            partial_rep_detected = False
                            partial_frames = down_frames = up_frames = 0

                    if movement_started:
                        # Top position (arms extended, near baseline)
                        if elbow_angle > 155 and vertical_drop < 0.02:
                            down_frames += 1
                            up_frames = 0
                            partial_frames = 0
                        ...
                        # Bottom position (chin over bar OR sufficient drop)
                        chin_over_bar = bar_locked and bar_y is not None and nose_y < bar_y - 0.005
                        deep_pull = vertical_drop > 0.04 and elbow_angle < 110
                        if chin_over_bar or deep_pull:
                            up_frames += 1
                            down_frames = 0
                            partial_frames = 0
                        ...
                        # Mid-range/partial
                        elif (80 <= elbow_angle <= 150 or vertical_drop > 0.02) and stage == "down":
                            partial_frames += 1
                            down_frames = 0
                            up_frames = 0
                ...
                # (alert drawing moved after video placement for visibility)
        except Exception as e:
        ...
        # Bar status hint
        hint_y = 50
        status_text = "Bar: locked" if bar_locked else "Bar: estimating... Hang still"
        status_color = COLOR_GREEN if bar_locked else COLOR_YELLOW
        cv2.putText(ui_image, status_text,
                    (VIDEO_WIDTH // 2 - 120, hint_y),
        ...
        cv2.putText(ui_image, "Pull until chin clears the bar; control the descent.",
                    (30, banner_y + 28),
        ...
        if key == ord('q') or key == ord('e'):
            ...
            show_exercise_menu = True
            exercise_menu_start_time = current_time
        # Dismiss instruction popup
        if key == ord('i'):
            instruction_popup_active = False
            consecutive_incorrect = 0

        cv2.imshow('FitMaster AI - Pull-Up Counter', ui_image)
        out.write(ui_image)
