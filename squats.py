...
recording_name = f"squat_session_{datetime.now().strftime('%Y%m%d_%H%M%S')}.mp4"
out = cv2.VideoWriter(recording_name, fourcc, 20.0, (WINDOW_WIDTH, WINDOW_HEIGHT))

# Create window and set mouse callback
cv2.namedWindow('FitMaster AI - Squat Counter')
cv2.setMouseCallback('FitMaster AI - Squat Counter', mouse_callback)
...
with mp_pose.Pose(min_detection_confidence=0.7, min_tracking_confidence=0.7) as pose:
    while cap.isOpened():
        ...
        try:
            ...
            # Draw angle text near knees on video frame
            ...
            if not is_paused:
                ...
                if delta_angle > movement_threshold or knee_angle < 150:
                    ...
                    if movement_idle_frames > movement_idle_limit:
                        movement_started = False
                        partial_rep_detected = False
                    ...
                    if knee_angle > 168:
                        ...
                        if down_frames >= frame_threshold and stage == "up":
                            if current_time - last_rep_time > min_time_between_reps:
                                ...
                                # reset partial/flags after a good rep
                                partial_rep_detected = False
                                partial_down_frames = 0
                        ...
                        if partial_down_frames >= frame_threshold:
                            partial_rep_detected = True  # Mark that a partial/incorrect rep started
                    # If moving up after a full rep, clear partial flags
        ...
        # Moved here: Connection Status (further right, not on video)
