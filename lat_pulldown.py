...
incorrect_counter = 0
sets = 1
stage = "up"  # start with arms extended overhead
frame_threshold = 2
up_frames = 0
down_frames = 0
partial_frames = 0
...
selected_exercise = "lat pulldown"
...
recording_name = f"lat_pulldown_session_{datetime.now().strftime('%Y%m%d_%H%M%S')}.mp4"
out = cv2.VideoWriter(recording_name, fourcc, 20.0, (WINDOW_WIDTH, WINDOW_HEIGHT))

cv2.namedWindow('FitMaster AI - Lat Pulldown Counter')
    ...
    while cap.isOpened():
        ...
        try:
            ...
            if landmarks:
                ...
                mid_x = int(((l_wrist[0] + r_wrist[0]) / 2) * VIDEO_WIDTH)
                mid_y = int(avg_wrist_y * VIDEO_HEIGHT)
                cv2.putText(video_image, f"Elbow {int(elbow_angle)}°",
                            (mid_x - 20, mid_y - 10),
                            cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0, 255, 255), 2)

                if not is_paused:
                    if elbow_angle > 150 and avg_wrist_y < avg_shoulder_y - 0.02:
                        up_frames += 1
                        down_frames = 0
                        partial_frames = 0
                        if up_frames >= frame_threshold:
                            if partial_rep_detected and current_time - last_rep_time > min_time_between_reps:
                                ...
                                partial_rep_detected = False
                            stage = "up"
                    elif 70 <= elbow_angle <= 120 and avg_wrist_y > avg_shoulder_y + 0.05:
                        down_frames += 1
                        up_frames = 0
                        partial_frames = 0
        ...
        cv2.putText(ui_image, "Drive elbows down and stop when bar reaches upper chest.",
                    (30, banner_y + 28),
